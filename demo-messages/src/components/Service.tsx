...
const Service = ({ id }: ServiceProperties) => {
  // Hooks //
  ...
  const service = useWardService(id, (message: Message) =>
  ...
  return (
    ...
    >
      ...
      <div>
        <label style={{ width: "4rem", display: "inline-block" }}>
          Payload
        </label>
        ...
        {messages.map((message, index) => {
          return (
            <li key={`event-${index}`}>
              ...
              <div>{JSON.stringify(message.payload, null, 2)}</div>
            </li>
          );
        })}
      </ul>
